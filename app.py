...
fig_pie.add_trace(
    go.Pie(labels=['Male', 'Female'], values=[84, 16]),
    row=1,
    col=2,
)
...
data_viz_page = html.Div([
    ...
    html.P('For our group project, we decided to focus on the discrepancy of representation of male and female characters in popular movies from the current day. To conduct our research, we collected information from the top 10 blockbusters of 2016, specifically looking at the number of large roles portrayed as male and female. Eight of the ten movies were dominated by dialogue from male characters with five of them featuring over 75% male dialogue characters. In fact, The Jungle Book featured 90% male dialogue compared to only 10% female dialogue, making the discrepancy visible to the uninterested eye. The remaining two movies were close to 50% male and female dialogue , with one of them favoring more female dialogue than male dialogue. These results were not necessarily surprising to us when considering the films that we analyzed. Six of the ten can be generalized as action movies and it is typical for these movies to exhibit male lead and support roles as that is what the industry as a whole has subscribed to. The other four can be generalized as family movies, two of which fall near to 50/50 representation. The bubble chart provides more interesting analysis for this group of films as well. We can see that overall, these films contain mostly male characters. Additionally, in these movies, male characters not only have a larger percentage of the dialogue, but on average they also speak more than female characters. Alarmingly, Finding Dory is the only film in the entire group that has a female character with the most turns speaking and most dialogue. Overall, the top 10 blockbusters of 2016 underrepresented female characters as male roles heavily dominated the big screen.', style={'text-align': 'center', 'margin-left': '5%', 'margin-right': '5%', 'margin-bottom': '5%'}),
    dbc.Row([
